...
function fun()
{
    ...
    img.src = "https://www.freeiconspng.com/thumbs/pacman-png/pacman-red-png-22.png";
    img.alt = "Random Image";
    img.width = 50; // Set image width
    img.height = 50; // Set image height
    // (table.rows[z].cells[y]);
    let cells = table.rows[z].cells[y];
    // console.log(cells);
    cells.appendChild(img);

    let h=Math.floor(Math.random()*n);
    console.log(h);
    ...
    //             newRow = Math.min(currentRow + 1, n - 1);
    //         } else if (event.key === 'ArrowLeft') {
    //             newCol = Math.max(currentCol - 1, 0);
    //         } else if (event.key === 'ArrowRight') {
    //             newCol = Math.min(currentCol + 1, n - 1);
    //         }
